...
def getVeryMiniTestDataset():
    ...
    # Prepare to collect 1 images per class
    ...
    for (image, label) in test_loader:
        ...
        if all(len(images) == 1 for images in class_images):
            break  # Stop once we have 1 images per class
...
def main():
    ...
    with torch.no_grad():
        ...
        run_serial_with_profiler(model=model, imgs=images)
